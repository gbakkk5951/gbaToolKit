...
void process(){
	if(access(org.c_str(),0)==-1){
		stroutput="缺少文件"+org+",请将需要测试的源程序粘贴至此目录并更改名称";
		cerr<<stroutput<<endl;
		return;
	}
	int i;
	...
	if(access(codeconst.c_str(),0)==-1){
		stroutput="缺少文件"+codeconst+",请在BackUp文件夹中找到并复制到运行目录下";
		cerr<<stroutput<<endl;
		return;
	}
...
void print(){
	if(access(result.c_str(),0)==-1){
		stroutput="缺少文件"+result+",可能因为未编译运行生成的Cpp";
		cerr<<stroutput<<endl;
		return;
	}
	int i,cnt;int tmp;
	freopen(result.c_str(),"r",stdin);
	cin>>tmp;
	if(tmp!=totcnt){
